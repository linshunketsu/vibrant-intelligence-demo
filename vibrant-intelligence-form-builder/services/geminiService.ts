...
import { FormField, FormFieldType, PersonalInfoSelection, MedicationHistorySelection, HealthInsuranceSelection, VitalsSelection, PaymentDetailsSelection, UniversalAgreementSelection } from '../types';
import { personalInfoLabels, medicationHistoryLabels, healthInsuranceLabels, vitalsLabels } from '../constants';

// Per guidelines, initialize with named parameter and API_KEY from env var.
const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

// Type definition for EHRMappingSuggestion, needed by EHRMappingModal.tsx
...
export async function generateFormWithAI(formPurpose: string, formDescription: string, existingFields: FormField[]): Promise<FormField[]> {
    const prompt = `
    ...
    try {
        ...
        // ensuring they are always fully formed regardless of AI output.
...
export async function suggestEHRMappings(fields: FormField[]): Promise<EHRMappingSuggestion[]> {
    if (fields.length === 0) {
        return [];
    }

...
// Renders form fields for preview/editing
// Migrated from vibrant-intelligence-form-builder with Tailwind CSS

import React, { useState, useRef, DragEvent } from 'react';
import { FormField, FormFieldType, EHRMapping, EHRMappingSuggestion } from '../../formTypes';
import { EHR_FIELD_PATHS } from '../../formConstants';
import { ChevronDown, Link2, Sparkles, Loader2, CreditCard, Building2 } from 'lucide-react';
...
const MappingIndicator: React.FC<{
  ...
  // Mapped and suggested indicators are always visible; unmapped only shows on hover
  const getVisibilityClasses = () => {
    if (variant === 'mapped') return 'opacity-100';
    if (variant === 'suggested') return 'opacity-100';
    if (variant === 'analyzing') return 'opacity-100';
    return 'opacity-0 group-hover:opacity-100'; // unmapped
  };
  ...
  const variantConfig = {
    ...
    suggested: {
      classes: 'bg-blue-100 text-blue-600 hover:bg-blue-200 animate-pulse',
  ...
  return (
    <button
      onClick={onClick}
      title={tooltipText}
      className={`${baseClasses} ${getVisibilityClasses()} ${variantConfig[variant].classes}`}
    ...
    <div>
      ...
      {imagePreview ? (
        ...
        <div
          ...
          className={`mt-1 w-full aspect-video border-2 border-dashed rounded-md flex flex-col items-center justify-center cursor-pointer transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${isDraggingOver ? 'border-blue-500 bg-blue-50' : 'border-slate-300 bg-slate-50 hover:bg-slate-100'}`}
        >
  ...
  switch (type) {
      ...
      return (
        <div className="group">
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm disabled:bg-slate-50 disabled:text-slate-500"
            disabled
          />
          {renderHelpText()}
        </div>
      ...
      return (
        <div className="group">
          <div className="flex items-center gap-2">
            <input type="checkbox" className="h-4 w-4 text-blue-600 border-slate-300 rounded focus:ring-2 focus:ring-blue-500 disabled:opacity-50" disabled />
          ...
          <div className="space-y-2 mt-1">
            {options?.map((opt, index) => (
              <div key={index} className="flex items-center">
                <input type="checkbox" name={`${field.id}-${index}`} className="h-4 w-4 text-blue-600 border-slate-300 rounded focus:ring-2 focus:ring-blue-500 disabled:opacity-50" disabled />
      ...
      return (
        <div className="group">
          ...
          <div className="space-y-2 mt-1">
            {options?.map((opt, index) => (
              <div key={index} className="flex items-center">
                <input type="radio" name={field.id} className="h-4 w-4 text-blue-600 border-slate-300 focus:ring-2 focus:ring-blue-500 disabled:opacity-50" disabled />
                <label className="ml-2 block text-sm text-slate-900">{opt}</label>
              </div>
            ))}
          </div>
      ...
      return (
        <div className="group">
          ...
          <div className="relative">
            <select className="w-full appearance-none bg-white px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled>
    ...
    case FormFieldType.SECTION_HEADER:
      return (
        <div className="pb-2 border-b border-slate-200">
          <h2 className="text-lg font-bold text-blue-700">{label}</h2>
          {renderHelpText()}
        </div>
      );
    case FormFieldType.COMPANY_HEADER:
      return (
        <div className="flex flex-col items-center gap-2 p-2">
          ...
          <h2 className="text-2xl font-bold text-slate-800 text-center">{label}</h2>
        </div>
      );
    ...
    case FormFieldType.EMAIL:
      return (
        <div className="group">
          {renderLabelWithIndicator()}
          <input type="email" placeholder={placeholder} className="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled />
          {renderHelpText()}
        </div>
      );
    case FormFieldType.PHONE:
      return (
        <div className="group">
          {renderLabelWithIndicator()}
          <input type="tel" placeholder={placeholder} className="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled />
          {renderHelpText()}
        </div>
      );
    case FormFieldType.DATETIME_PICKER:
      return (
        <div className="group">
          {renderLabelWithIndicator()}
          <input type="datetime-local" className="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled />
          {renderHelpText()}
        </div>
      );
    case FormFieldType.TIME_PICKER:
      return (
        <div className="group">
          {renderLabelWithIndicator()}
          <input type="time" className="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled />
          {renderHelpText()}
        </div>
      );
    case FormFieldType.RICH_TEXT:
      return (
        <div
          className="rich-text-content text-sm text-slate-800 prose prose-sm max-w-none"
      ...
      return (
        <div>
          <div className="pb-2 mb-4 border-b border-slate-200">
            <h2 className="text-lg font-bold text-blue-700">{label}{required && <span className="text-red-500 ml-1">*</span>}</h2>
            {renderHelpText()}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {field.personalInfo?.includeFullName && <SubField label="Full Name" fieldId={field.id} subFieldKey="includeFullName" confirmedMappings={confirmedMappings} suggestedMappings={suggestedMappings} analyzingMappingKeys={analyzingMappingKeys} onViewMapping={onViewMapping} onViewSuggestion={onViewSuggestion}><input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></SubField>}
            ...
            {field.personalInfo?.includeAddress && (
              <div className="sm:col-span-2 space-y-4 p-4 bg-slate-100 rounded-md border border-slate-200">
                <SubField label="Address Block" fieldId={field.id} subFieldKey="includeAddress" confirmedMappings={confirmedMappings} suggestedMappings={suggestedMappings} analyzingMappingKeys={analyzingMappingKeys} onViewMapping={onViewMapping} onViewSuggestion={onViewSuggestion}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="sm:col-span-2"><div><label className="block text-sm font-medium text-slate-700 mb-1">Address Line 1</label><input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div></div>
                    <div className="sm:col-span-2"><div><label className="block text-sm font-medium text-slate-700 mb-1">Address Line 2</label><input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div></div>
                    <div><label className="block text-sm font-medium text-slate-700 mb-1">City</label><input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div>
                    <div><label className="block text-sm font-medium text-slate-700 mb-1">State / Province</label><input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div>
                    <div><label className="block text-sm font-medium text-slate-700 mb-1">Zip / Postal Code</label><input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div>
                  </div>
                </SubField>
              </div>
            )}
            ...
            {field.personalInfo?.includeContactInfo && (
              <div className="sm:col-span-2 space-y-4 p-4 bg-slate-100 rounded-md border border-slate-200">
                <SubField label="Contact Info Block" fieldId={field.id} subFieldKey="includeContactInfo" confirmedMappings={confirmedMappings} suggestedMappings={suggestedMappings} analyzingMappingKeys={analyzingMappingKeys} onViewMapping={onViewMapping} onViewSuggestion={onViewSuggestion}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label className="block text-sm font-medium text-slate-700 mb-1">Email Address</label><input type="email" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div>
                    <div><label className="block text-sm font-medium text-slate-700 mb-1">Phone Number</label><input type="tel" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div>
                  </div>
                </SubField>
              </div>
            )}
      ...
      return (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {field.healthInsurance?.includePhoneNumber && <SubField label="Insurance Phone Number" fieldId={field.id} subFieldKey="includePhoneNumber" confirmedMappings={confirmedMappings} suggestedMappings={suggestedMappings} analyzingMappingKeys={analyzingMappingKeys} onViewMapping={onViewMapping} onViewSuggestion={onViewSuggestion}><input type="tel" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></SubField>}
            {field.healthInsurance?.includePolicyHolder && <SubField label="Relationship to Patient" fieldId={field.id} subFieldKey="includePolicyHolder" confirmedMappings={confirmedMappings} suggestedMappings={suggestedMappings} analyzingMappingKeys={analyzingMappingKeys} onViewMapping={onViewMapping} onViewSuggestion={onViewSuggestion}><input type="text" placeholder="e.g., Self, Spouse, Parent" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></SubField>}
      ...
      return (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4">
            {field.vitals?.includeHeight && <SubField label="Height (cm)" fieldId={field.id} subFieldKey="includeHeight" confirmedMappings={confirmedMappings} suggestedMappings={suggestedMappings} analyzingMappingKeys={analyzingMappingKeys} onViewMapping={onViewMapping} onViewSuggestion={onViewSuggestion}><input type="number" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></SubField>}
            ...
            {field.vitals?.includeBloodPressure && <div className="sm:col-span-2"><SubField label="Blood Pressure" fieldId={field.id} subFieldKey="includeBloodPressure" confirmedMappings={confirmedMappings} suggestedMappings={suggestedMappings} analyzingMappingKeys={analyzingMappingKeys} onViewMapping={onViewMapping} onViewSuggestion={onViewSuggestion}><div className="flex items-center gap-2"><div><label className="block text-sm font-medium text-slate-700 mb-1">Systolic (mmHg)</label><input type="number" placeholder="Systolic" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div><span className="text-slate-500 pt-6">/</span><div><label className="block text-sm font-medium text-slate-700 mb-1">Diastolic (mmHg)</label><input type="number" placeholder="Diastolic" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div></div></SubField></div>}
          </div>
        </div>
      );
            ...
            {field.paymentDetails?.includeCardDetails && (
              <div>
                ...
                <div className="flex items-center gap-0 border border-slate-300 rounded-md shadow-sm bg-white">
                  <span className="pl-3 text-slate-400"><CreditCard className="h-4 w-4" /></span>
                  <input type="text" placeholder="Card number" className="flex-grow px-3 py-2 border-0 rounded-l-md sm:text-sm focus:ring-0 disabled:bg-transparent disabled:text-slate-400" disabled />
                  <input type="text" placeholder="MM / YY" className="w-24 px-3 py-2 border-0 border-l border-slate-300 text-center sm:text-sm focus:ring-0 disabled:bg-transparent disabled:text-slate-400" disabled />
                  <input type="text" placeholder="CVC" className="w-20 px-3 py-2 border-0 border-l border-slate-300 rounded-r-md text-center sm:text-sm focus:ring-0 disabled:bg-transparent disabled:text-slate-400" disabled />
                </div>
              </div>
            )}
            ...
            {field.paymentDetails?.includeBillingAddress && (
              <div className="space-y-4 p-4 bg-slate-100 rounded-md border border-slate-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div><label className="block text-sm font-medium text-slate-700 mb-1">State / Province</label><input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div>
                  <div><label className="block text-sm font-medium text-slate-700 mb-1">Zip / Postal Code</label><input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div>
                  <div><label className="block text-sm font-medium text-slate-700 mb-1">Country</label><input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled /></div>
                </div>
              </div>
            )}
          </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            <div className="flex items-start">
              <input type="checkbox" id={`${field.id}_agree`} className="h-4 w-4 text-blue-600 border-slate-300 rounded focus:ring-2 focus:ring-blue-500 mt-0.5 disabled:opacity-50" disabled />
            ...
            {field.universalAgreement?.includePrintedName && (
              <div>
                ...
                <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled />
              </div>
            )}
            {field.universalAgreement?.includeDate && (
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Date</label>
                <input type="date" className="w-full px-3 py-2 border border-slate-300 rounded-md sm:text-sm disabled:bg-slate-50 disabled:text-slate-500" disabled />
              </div>
            )}
          </div>
        </div>

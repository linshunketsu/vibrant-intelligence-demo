...
import { TimeRange, UserRole, AtRiskPatient, Transaction, PaymentDetail, PracticeDocument } from '../types';
import { Card } from '../components/Card';
import { FilterIcon, ChevronDownIcon, SparklesIcon, GearIcon, WarningIcon, SendIcon, SearchIcon, DocumentAddIcon, FileIcon, DownloadIcon, TruckIcon, CalculatorIcon, BookOpenIcon, CubeIcon, ClipboardIcon, ChevronRightIcon, DragHandleIcon, PlusIcon, CheckCircleIcon, XMarkIcon, TrashIcon, PinIcon, CheckIcon, CalendarIcon, CreditCardIcon, BuildingLibraryIcon, WalletIcon, ChatBubbleIcon, ArrowPathIcon, DotsHorizontalIcon, InformationCircleIcon, ArrowPathIcon as LoadingIcon, ArrowUpIcon, ArrowDownIcon, BeakerIcon, EyeIcon, EyeSlashIcon, UserIcon } from '../components/Icons';
import { MOCK_PRACTICE_WALLET, MOCK_AT_RISK_PATIENTS, MOCK_TRANSACTIONS, MOCK_DOCUMENTS, MOCK_PROVIDERS, METRIC_LIBRARY_DATA } from '../constants';
...
import { analyzeEngagement, queryPracticeInsights } from '../services/geminiService';
...
const SidePanel: React.FC<SidePanelProps> = ({ title, isOpen, onClose, children, className = "max-width-md", footer }) => {
...
const FilterDropdown: React.FC<FilterDropdownProps> = ({ label, options, value, onChange }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute top-full left-0 mt-2 w-full min-width-[180px] bg-white rounded-lg shadow-xl ring-1 ring-black/5 border border-slate-100 py-1.5 z-50 animate-in fade-in zoom-in-95 duration-100 origin-top-left">
                    {options.map((option) => (
                        <button
                            key={option}
                            onClick={() => {
...
const UploadDocumentModal = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Upload Document" size="2xl">
      <div className="space-y-6">
        <div>
          ...
          <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 flex flex-col items-center justify-center text-center hover:bg-slate-50 transition-colors relative">
            {file ? (
              <div className="flex items-center justify-between w-full max-width-md bg-brand-light/30 p-4 rounded-lg border border-brand-primary/20">
                <div className="flex items-center space-x-3">
                  <div className="p-2 bg-white rounded-lg shadow-sm">
                    <FileIcon className="h-6 w-6 text-brand-primary" />
                  </div>
                  <div className="text-left">
                    <p className="text-sm font-semibold text-slate-900 truncate max-width-[200px]">{file.name}</p>
...
const Dashboard: React.FC<DashboardProps> = ({ onOpenSettings, currentUserRole, setCurrentUserRole }) => {
  ...
  // Admins can toggle 'all' (Practice) or a specific provider. Providers are locked to their own.
  const [activeProviderId, setActiveProviderId] = useState<string | 'all'>(
    currentUserRole === UserRole.Provider ? '1' : 'all'
  );

  // -- KPI PINNING & KEY METRICS LOGIC --
  ...
  return (
    <div className="space-y-6 pb-12">
             ...
             <div className="flex items-center space-x-2">
                ...
                <div className="relative">
                    <button 
                        ...
                        className={`flex items-center space-x-2 bg-white border ${isScopeDropdownOpen ? 'border-brand-primary ring-2 ring-brand-primary/10' : 'border-slate-300 hover:border-slate-400'} rounded-lg py-1.5 pl-3 pr-2 text-xs font-semibold text-slate-700 shadow-sm transition-all min-width-[160px]`}
                    >
                ...
                <div className="flex items-center space-x-4">
                    <div className="relative">
                        ...
                        <button 
                            ...
                            className={`group flex items-center justify-between bg-white border ${isDateFilterOpen ? 'border-brand-primary ring-2 ring-brand-primary/10' : 'border-slate-200 hover:border-slate-300'} py-2.5 pl-4 pr-3 rounded-lg text-sm font-medium text-slate-700 focus:outline-none transition-all shadow-sm min-width-[200px]`}
                        >
                        ...
                        {isDateFilterOpen && (
                            <div className="absolute top-full left-0 mt-2 w-full min-width-[220px] bg-white rounded-lg shadow-xl ring-1 ring-black/5 border border-slate-100 py-1.5 z-50 animate-in fade-in zoom-in-95 duration-100 origin-top-left">
                                ...
                                <Tooltip 
                                    ...
                                    content={({ active, payload, label }) => {
                                        if (active && payload && payload.length) {
                                            const data = payload[0].payload;
                                            return (
                                                <div className="bg-white border border-slate-200 shadow-xl rounded-xl p-4 text-xs min-width-[240px]">
                        ...
                        return (
                            <React.Fragment key={t.id}>
                                <tr className={`group transition-colors ${selectedTxIds.has(t.id) ? 'bg-brand-light/20' : 'hover:bg-slate-50/80'} ${isExpanded ? 'bg-slate-50/50' : ''}`}>
                                    ...
                                    <td className="px-4 py-4 whitespace-nowrap"><span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border ${statusColor[t.status]}`}>{t.status}</span></td>
                                    ...
                                    <tr className="bg-slate-50/30">
                                        <td colSpan={12} className="px-12 py-4 border-l-4 border-brand-primary">
                                            <div className="animate-in slide-in-from-top-2 duration-300">
                                                ...
                                                <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm max-width-4xl">
                                                    <table className="min-w-full divide-y divide-slate-100">
      ...
      <SidePanel title="Follow-ups" isOpen={isPanelOpen} onClose={() => setIsPanelOpen(false)} className="max-width-lg" footer={<div className="text-xs text-slate-500">{displayedFollowUps.length} items for this scope</div>}>
      ...
      {/* Side Panel as a global filter selector */}
